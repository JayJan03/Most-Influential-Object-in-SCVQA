...
class SelfCriticalDataset(Dataset):
    def __init__(self, name, dictionary, opt, dataroot='data'):
        ...
        assert name in ['v2cp_train', 'v2cp_test', 'v2cp_train_hat', 'v2cp_test_hat', 
             'v2cp_train_vqx', 'v2cp_test_vqx']
        self.name = name
        ...
        if name == 'v2cp_test' or name == 'v2cp_train':
            if name == 'vg':
                self.entriess = cPickle.load(open(dataroot + '/VG_dataset.pkl', 'rb'))
            else:
                if name == 'v2cp_test':
                     self.entriess = cPickle.load(open(dataroot + '/VQAcp_caption_' + 'test' + 'dataset.pkl', 'rb'))
                else:
                    self.entriess = cPickle.load(open(dataroot + '/VQAcp_caption_' + 'train' + 'dataset.pkl', 'rb'))
            self.entries = {}
            count = 0
...
class SelfCriticalDataset1(Dataset):
#trying to subset dataset to be tested
 
    def __init__(self, name, dictionary, opt, dataroot='data'):
        super(SelfCriticalDataset1, self).__init__()
        assert name in ['v2cp_train', 'v2cp_test', 'v2cp_train_hat', 'v2cp_test_hat', 
             'v2cp_train_vqx', 'v2cp_test_vqx']
        self.name = name
        self.dictionary = dictionary  # questions' dictionary
        ...
        if name == 'v2cp_test' or name == 'v2cp_train':
            ...
            self.entriess = self.entriess[0:300]
            self.entries = {}
            count = 0
            self.train_hint = cPickle.load(open(os.path.join(dataroot, 'train_cp_hint.pkl'), 'rb'))
            self.val_hint = cPickle.load(open(os.path.join(dataroot, 'val_cp_hint.pkl'), 'rb'))
    ...
    def __getitem__(self, index):
        ...
        return obj_nodes, question, target, hint_score, hint_o, qid, imgid
