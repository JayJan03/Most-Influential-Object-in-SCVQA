...
def evaluate(model, dataloader):
    ...
    ff = open("/home/cvpr/jayant/bottom-up-attention-master/merged_cf10.pkl", "rb")					#change path
    u = pickle._Unpickler( ff )
    u.encoding = 'latin1'
    loadboxes = u.load()
    qids_ps = list(loadboxes.keys())
    ...
    for objs, q, a, hintscore, _, qids,imgid in tqdm(iter(dataloader)):
        ...
        for jj in range(len(qids)):
            if qids[jj] in qids_ps:
                objs[jj] = loadboxes[qids[jj]]['features']
        ...
        #qids = qids.detach().cpu().int().numpy()
        for j in range(len(qids_ps)):
            qid = qids_ps[j]
            print(qid)
            typ = qid2type[qid]
            #typ = 'yes/no'				#just a shortcut to bypass qid2type as its probably only for test data and not train data

            if typ == 'yes/no':
                print(batch_score[qids.index(qid)])
                if batch_score[qids.index(qid)] >= 0.5:
                    new_ent = { 'image_id': imgid[j], 'qid': qid}
                else:
                    new_ent = { 'image_id': imgid[j], 'qid': qid}
                score_yesno += batch_score[qids.index(qid)]
                total_yesno += 1
            elif typ == 'other':
                print(batch_score[qids.index(qid)])
                if batch_score[qids.index(qid)] >= 0.5:
                ...
                score_other += batch_score[qids.index(qid)]
                total_other += 1
            elif typ == 'number':
                print(batch_score[qids.index(qid)])
                if batch_score[qids.index(qid)] >= 0.5:
                    ...
                    new_ent = { 'image_id': imgid[j], 'qid': qid, 'qtype': typ, 'match': "no"}
                score_number += batch_score[qids.index(qid)]
                total_number += 1
            else:
    ...
    #score = score / len(qids_ps)
    #V_loss /= len(dataloader.dataset)

    print("total_yesno::::",total_yesno)
    ...
    #pickle.dump(att, open('att_gv_100_vqx_ps.pkl', 'wb'))
    #pickle.dump(dict, open('eval_100_vqx_ps.pkl', 'wb'))
