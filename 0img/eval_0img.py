#creating 0 images.. all features are noise
...
def evaluate(model, dataloader):
    ...
    for objs, q, a, hintscore, _, qids,imgid in tqdm(iter(dataloader)):
        ...
        infind = []
        att100 = pickle.load(open('att_gv_100.pkl', 'rb'))
        for jjj in range(len(att100[0])):
            infind.append(att100[0][jjj].argmax().item())

        for jj in range(len(objs)):
            for kkk in range(36):            
                for kk in range(2048):            
                    objs[jj][kkk][kk] = np.random.rand(1)		#all features to noise
    ...
    pickle.dump(dict, open('eval_100_0img.pkl', 'wb'))

    pickle.dump(att, open('att_gv_100_0img.pkl', 'wb'))
...
if __name__ == '__main__':
    ...
    opt.use_all = 1
    eval_dset = SelfCriticalDataset1('v2cp_test', dictionary, opt)
